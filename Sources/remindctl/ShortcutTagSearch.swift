...
enum ShortcutTagSearch {
  ...
  private static let category = "search"
  private static let installGuidance = "Reinstall the bundled .shortcut file and see the README."
  ...
  static func search(tags rawTags: [String]) throws -> [ShortcutTagReminder] {
    ...
    guard payload.success else {
      ...
      throw RemindCoreError.operationFailed(
        "Shortcut \"\(shortcutName)\" reported failure: \(detail). \(installGuidance)"
      )
    }
    guard payload.count == payload.data.count else {
      throw RemindCoreError.operationFailed(
        "Shortcut \"\(shortcutName)\" returned an invalid count (\(payload.count) vs \(payload.data.count)). \(installGuidance)"
      )
    }
  ...
  private static func processFailure(_ result: ProcessResult, outputURL: URL) -> Error {
    ShortcutProcessErrorFormatter.processFailure(
      ...
      result: result,
      outputURL: outputURL,
      missingShortcutGuidance: "Shortcut \"\(shortcutName)\" is required for --tag searches. \(installGuidance)",
      installGuidance: installGuidance
    )
  }
}
